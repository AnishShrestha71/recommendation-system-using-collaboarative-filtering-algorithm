...
<body>
    ...
    <div class="form">
        <p class="text-secondary line fs-4 mb-4">Hey, <?php echo $_SESSION['username']; ?>!</p>
         
        <p class="text-secondary line fs-5" ">Your Profile:</p>
		...
		<div class="single-tutor">
		 ...
		 <?php $uemail= $row["email"] ?>

		 <?php $uloc= $row["Location"] ?>
		 <p class="text-secondary d-flex fs-5 line"><span>Email: </span>&nbsp;<?php  echo  $row["email"] ?></p>
		 <p class="text-secondary  d-flex fs-5 line"><span>Location: </span>&nbsp;<?php echo $row["Location"] ?></p>
		 <p class="text-secondary  d-flex fs-5 line"><span>Subject: </span>&nbsp;<?php echo $row["Subject"] ?></p>
		
		<?php $student_prefer_subject = $row["Subject"] ?>
		...
		<a style="text-decoration:none" class=" text-light" href="update.php?location=<?php  echo $uloc ?>&email=<?php  echo $uemail ?>&subject=<?php  echo $student_prefer_subject ?>">			
 ...
 -->
	...
	?>
 	<h1 class="mx-4  mb-1">Recommended teachers</h1>
	...
	?>
   		...
   		<?php
   		$sql = "SELECT *  FROM  treg WHERE username='$teacher' AND Subject LIKE '%$student_prefer_subject%' AND Experience >= '2' AND Average >= '60'";
   		$result = $con->query($sql);
...
<h1  class="mx-4 mb-1">Teachers of <?php echo $loc; ?></h1>

<?php //echo $loc; 
										
	include("Sdb.php");
										
			$sql = "SELECT *  FROM  treg WHERE Location LIKE '%$loc%' ";
			$result = $con->query($sql);
			if ($result->num_rows > 0) {
...
<h1  class="mx-4  mb-1">Teachers available</h1>
